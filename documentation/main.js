...
const features = [
    ...
    new Feature("each", "Loop through a list or object without unnecessary syntax. 'Each' is similar to 'for of' in JavaScript or 'foreach' in C#, for example. The provided variable will be defined for every item in the list and can be used in the following scope.", "each(%variable name% of %expression%) {%any number of expressions%}", "each(number of [0, 1, 2, 3, 4]) {\n\xa0holler(number)\n}\n# output: 0\n1\n2\n3\n4 #", ["for", "while"]),
    ...
    new Feature("Inheritance", "Classes may extend other classes, just like objects can extend classes. This allows objects of said class to reference properties and methods from either class they inherit from.", "class after %name of other class (string)% {%any number of expressions%}", "dig Organism plant class {\n\xa0dig hasRibosomes plant yes\n}\ndig Person plant class after Organism {\n\xa0dig hasTeeth plant yes\n}\ndig dak plant new Person()\nholler(dak.hasTeeth) # uses directly inherited property #\nholler(dak.hasRibosomes) # uses indirectly inherited property #\n# output: yes\nyes #", ["class", "Object syntax", "super"]),
    ...
    new Feature("switch", "This keyword allows the user to execute a block of code section by section as various values are checked against an original in a linear fashion. Execution will begin once a case is found to be equal to the original value, and end once an end, cancel, or harvest is detected. If a default block is provided, it will run after all cases.", "switch(%expression%) {%any number of expressions in between one or more case %expression% statements and an optional default statement%}", "switch(12) {\n\xa0case 11\n\xa0holler(\"nah\")\n\xa0case 12\n\xa0holler(\"yup\")\n\xa0case 13\n\xa0holler(\"also yup\")\n\xa0cancel\n\xa0case 14\n\xa0holler(\"nope\")\n\xa0default\n\xa0holler(\"also nope\")\n}\n# output: yup\nalso yup #", ["end, cancel, continue", "harvest | h"]),
